    ...
    tinymce.init({
        ...
        setup: function (editor) {
            editor.on('init', function () {
                ...
                // const meanDiv = document.querySelector(".mean-div");
                // if (meanDiv) {
                //     meanDiv.classList.remove("outline");
                // }
                // if (content) {
                //     editor.setContent(content);
                // }
                //
                // const cleanContent = editor.getContent({ format: 'text' });
                // totalWords = cleanContent.trim() === "" ? 0 : cleanContent.trim().split(/\s+/).filter(Boolean).length;
                //
                // responseText = cleanContent;
                //
                // // Initialize content array
                // contentArray = getContentArray(cleanContent);
                //
                // // Initial update when TinyMCE is initialized
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    convertToWordBtn.addEventListener('click', function () {
        ...
        if (editorContent !== '') {
            ...
            const file = new File([blob], 'converted_text.txt', { type: 'text/plain' });
...
$(document).on("click", ".js-download-all", (e) => {
    ...
    $(".js-single-img-preview").each(function (index, element) {
        ...
        if (!hasDiv) {
            ...
            if (TOOL_NAME === "JPGTOWORD" || TOOL_NAME === "PDFTOWORD" || TOOL_NAME === "TEXTTOWORD") {
                ext = ".doc";
            } else {
                ext = ".txt";
...
((e) => {
    ...
    //     $(wrap).html(`<span>Submit and Extract</span>`);
    //     const style = {
    //         padding: "4px 20px",
    //         borderRadius: "10px",
    //         background: "#0984E3",
    //         boxShadow: "0px 5px 13px 0px rgba(0, 39, 69, 0.10)",
    //         fontSize:"16px",
    //         color: "white",
    //         width: "fit-content",
    //         margin: "0 auto",
    ...
    //         let totalDivs ;
    //
    //         let PDFimg = `${BASE_URL}web_assets/frontend/img/pdf.png`;
    ...
    //         });
    //
    //         const ajaxCall = (data, divId , count) => {
    //             $.ajaxSetup({
    //                 headers: {
    ...
    //
    // // Update the corresponding div with response text
    //                                 if (typeof html !== 'string' || html.trim() === '' || html.length === 0) {
    //                                     $(`#${divId} .center-preview-file-uploadIcon`).html(
    //                                         `<div class="no-response-title">No Text Found</div>
    //          <div class="no-response-des">We could not find any text in this image.</div> `
    //                                     );
    //                                     $(`#${divId}`).css('border', '1px solid #DE1A1A');
    //                                     $(`#${divId} .right-preview`).html(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 44 44" fill="none">
    ...
    //     </svg>`);
    //                                     $(`#${divId} .right-preview`).css({
    //                                         top: "30px"
    //                                     });
    //                                 } else {
    //                                     // Get the target element
    //                                     const targetElement = $(`#${divId} .center-preview-file-uploadIcon`);
    ...
    //                                         overflow: 'auto', // Allow scrollbar in textarea
    //                                         height: '100%', // Make textarea take full height
    //                                         /* other styles if needed */
    //                                     });
    //
    //                                     $(`#${divId} .js-single-result-copy`).removeClass('disabled');
    //                                     $(`#${divId} .js-single-result-download`).removeClass('disabled');
    //                                 }
    //
    //                                 if (
    //                                     parseInt(count + 1) ==
    ...
    //             originalIndices.forEach(({file_name }) => {
    //
    ...
    //                 let divId = `js-single-img-preview-${count}`;
    //                 let html =
    //                     `<div id="${divId}" class="js-single-img-preview" style="min-height: 120px ; max-height: 120px">` +
    ...
    //
    //                     `</div>` +
    //                     `<div class="right-preview" style="flex-direction: column;right: 10px;
    //                                         position: absolute">` +
    //                     // `<span class='single-img-action-btns'>` +
    ...
    //                     `</div>`;
    //
    //                 // Append the div with image and count
    //                 $(".multiple-img-preview").append(html);
    //                 // Assuming you have a variable `progress` representing the progress percentage
    //                 let progress = 0;
    //                 const progressBar = $(`#${divId} .progress-bar2`);
    //                 const percentageElement = $(`#${divId} .percentage span`);
    //
    ...
    //                 ajaxCall(data, divId , count); // Send the FormData, the associated image URL, and the count
    //                 count++;
    //             });
    //
...
`;
  ...
  <path d="M21.9949 15V25.4M21.9949 25.4L18.7949 21.8M21.9949 25.4L25.1949 21.8" stroke="#0D1120" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        let downloadAllImgSrc = `${BASE_URL}web_assets/frontend/img/icon/download-all-icon.svg`;
        let TEXTimg = `${BASE_URL}web_assets/frontend/img/word-img-png.png`;
        $("#jsShadowRoot").addClass('d-none');
        // $(".img_loader").removeClass("d-none");
        $(".restart_btn").addClass("d-none");
        window.scrollTo({
        ...
        if (imgArr.length) {
            ...
            originalIndices.forEach(({ file_name }, index) => {
                ...
                if (existingDiv.length && existingDiv.find('.center-preview-file-bar').length > 0) {
                    ...
                    let imageURL = TEXTimg; // Create a URL for the image
