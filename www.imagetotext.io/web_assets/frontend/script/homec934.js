...
$(document).ready(function () {
    ...
    $(document).on("click", ".js-download-all", (e) => {
        ...
        $(".js-single-img-preview").each(function (index, element) {
            ...
            if (!hasDiv) {
                ...
                let ext = TOOL_NAME === "JPGTOWORD" ? ".doc" : ".txt";
                ext = (ext == '.html') ? '.txt' : ext;
    ...
    $(document).on('click', '.js-single-result-download', function () {
        ...
        var textarea = $(this).closest('.center-preview').find('.center-preview-file-uploadIcon')[0];
        var text = textarea.value;
        var blob = TOOL_NAME === "JPGTOWORD" ? new Blob([text], { type: 'application/msword' }) : new Blob([text], { type: "text/plain;charset=utf-8" });
        var url = URL.createObjectURL(blob);
        var divName = document.getElementsByClassName('full-name')[index_click_result];

        var downloadLink = document.createElement("a");
        downloadLink.href = url;
        downloadLink.download = TOOL_NAME === "JPGTOWORD" ? `${divName.innerText}.doc` : `${divName.innerText}.txt`;
    ...
    $("#color_mode").click(function () {
        if ($(this).prop("checked") == true) {
            $(".monthly_field").addClass("d-none");
            $(".yearly_field").removeClass("d-none");
        } else if ($(this).prop("checked") == false) {
            $(".yearly_field").addClass("d-none");
            $(".monthly_field").removeClass("d-none");
        }
    });
...
`;
    ...
    function shadowMainFunction() {
        ...
        $(".js-single-img-preview").each(function (index) {
            ...
            var file_name = $(this).find(".full-name").text(); // Extract file name
            if (file_name.endsWith('.html')) {
                file_name = file_name.slice(0, -5) + '.txt'; // Replace .html with .txt
            }
        ...
        originalIndices.forEach(({ file_name }, index) => {
            const existingDiv = $(".js-single-img-preview").eq(index); // Select the element based on index
        ...
        const ajaxCall = (data, imageURL, count) => {
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    ...
                    $(`.js-single-img-preview:has(img[src="${imageURL}"]) .loading-bar`).show();
                    $(".response-getting").removeClass("d-none");
                    $(".response-getting").empty().prepend(
                        `<div class="response_getting">
                                    Extracting...
                ...
                success: function (response) {
                    if (response) {
                        ...
                        $(`.js-single-img-preview:has(img[src="${imageURL}"]) .loading-bar`).hide();
                        totalResponses++; // Increment the total responses received
                        ...
                        if (response.error) {
                            ...
                            if (imgInfo) {
                                $(".center-preview-file-uploadIcon").css({
                                    color: '#252A3E',
                                });
                                let encodeText = `${response.text}`;
                                let html = decodeHTMLEntities(encodeText);
                                // let html = decodeHTMLEntities(encodeText).replace(/<\/?[^>]+(>|$)/g, "");
                                // let html = encodeText;
                                if (typeof html !== 'string' || html.trim() === '') {
                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .center-preview-file-uploadIcon`).html(
                                        `<div class="no-response-title">No Text Found</div>
                                        <div class="no-response-des">We could not find any text in this image.</div> `
                                    );

                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"])`).css('border', '1px solid #DE1A1A');
                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .right-preview`).html(`<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 44 44" fill="none">
                                    ...
                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .right-preview`).css({
                                        top: "30px"
                                    });
                                } else {
                                    // Get the target element
                                    var existingDiv = $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]`);
                                    ...
                                    const targetElement = $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .center-preview-file-uploadIcon`);
                                    ...
                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .center-preview`).css({
                                        position: "relative"
                                    });
                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .right-preview`).css({
                                    ...
                                    $(`.js-single-img-preview:has(img[src="${imgInfo.imageURL}"]) .right-preview`).html(`<span class="d-flex align-items-center js-single-result-copy title-hover" style="width: 40px" data-title="Copy">${copyImgSrc}</span>` +
                ...
                error: function (error) {
                    ...
                    $(".reset-btn").show();
                    $(`.js-single-img-preview:has(img[src="${imageURL}"]) .loading-bar`).hide();
                    $(`.js-single-img-preview:has(img[src="${imageURL}"]) .center-preview-file-uploadIcon`).html(
                        `<div class="no-response-title">Error!</div>
                        <div class="no-response-des">We encountered an error with your request... please try again.</div> `
                    );
        ...
        const mainProcess = () => {
            ...
            originalIndices.sort((a, b) => parseInt(a.originalIndex) - parseInt(b.originalIndex));
            let imageURLs = [];
            originalIndices.forEach(async ({ file_name, originalIndex }, index) => {
                // Append new div
                const file = imgArr.find((item) => (item.name == file_name));

                const existingDiv = $(".js-single-img-preview").eq(index); // Select the element based on index

                if (existingDiv.length && existingDiv.find('.center-preview-file-bar').length > 0) {
                    let data = new FormData();
                    const { base64String, fileType } = await fileToBase64(file);
                    data.append("data", base64String);
                    data.append("type", fileType);
                    // data.append("imgArr", file);
                    data.append("count", count);
                    data.append("req_key", CAPTCHA_TOKEN);
                    // data.append("token", $('[name="cf-turnstile-response"]').val());
                    let img_drag = `${BASE_URL}web_assets/frontend/img/drag.svg`;
                    let imageURL = URL.createObjectURL(file); // Create a URL for the image
                    imageURLs.push(imageURL);
                    // Update existing div

                    existingDiv.find('.left-preview-img').html(`<img src="${imageURL}" alt="uploaded image">`);
                    existingDiv.find('.center-preview-file-bar').remove();
                    existingDiv.find('.center-preview-file-uploadIcon').html(
                        `<div class="progress2 progress-moved">` +
                        `<div class="progress-bar2 progress-bar-${count}"></div>` +
                        `</div>` +
                        `<div class="percentage percentage-${count}"><span>0%</span> Converted </div>`
                    );
                    existingDiv.css({
                        margin: "0 5px"
                    });
                    existingDiv.find('.right-preview .delete-action').removeClass('delete-action');

                    existingDiv.find('.left-preview-img-drag').remove();
                    existingDiv.find('.cropper-image').addClass('zoom-img');
                    existingDiv.find('.cropper-image').removeClass('cropper-image');

                    let progress = 0;
                    const progressBar = $(".progress-bar-" + count);
                    const percentageElement = $(".percentage-" + count + " span");
                    ...
                    ajaxCall(data, imageURL, count); // Send the FormData, the associated image URL, and the count
                    count++;
    ...
    function fileToBase64(file) {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = function (event) {
                ...
                resolve({ base64String, fileType: file.type });
            };
